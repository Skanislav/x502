...
///
/// Reads RPC_URL from argv (or env), deploys mocks (USDC, AgentRegistry,
/// FactProvider, MockEAS) + vault, registers 3 agent keys in the registry,
/// configures the demo repo, mints + deposits USDC, and writes
/// `demo/.runtime/addresses.json` for downstream scripts.
///
/// The vault is wired to MockEAS for local mode. To run the demo against a
/// Base Sepolia fork (real EAS), use chunk 3's run-stack hook (anvil
/// --fork-url) and skip this seed's MockEAS deploy.
...
async function main() {
  ...
  process.stdout.write("[seed] deploying mocks + MockEAS + vault\n");
  ...
  process.stdout.write(`[seed]   eas=${contracts.eas}\n`);
  process.stdout.write(`[seed]   schemaUID=${contracts.schemaUID}\n`);
  ...
  const rt: DemoRuntime = {
    ...
    contracts: {
      ...
      factProvider: contracts.factProvider,
      vault: contracts.vault,
      eas: contracts.eas,
    },
    schemaUID: contracts.schemaUID,
    repo: {
      slug: REPO_SLUG,
      repoId,
      threshold,
