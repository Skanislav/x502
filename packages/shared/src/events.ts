...
/// Wire format for the demo "verifier theater" event stream. Both the
/// coordinator and each verifier-agent publish events; the coordinator's
/// `/events` SSE endpoint multiplexes its own events with re-published
/// verifier events.
...
export type DemoEvent =
  ...
  | { type: "verifier.started"; claimId: Hex; agentId: string; ts: number }
  | {
      type: "verifier.reasoning";
      claimId: Hex;
      agentId: string;
      thinkingChunk: string;
      final?: { accept: boolean; reason: string };
      ts: number;
    }
  | { type: "verifier.signed"; claimId: Hex; agentId: string; signature: Hex; ts: number }
  | { type: "verifier.rejected"; claimId: Hex; agentId: string; reason: string; ts: number }
...
/// it only runs in single-process coordinator/verifier instances.
